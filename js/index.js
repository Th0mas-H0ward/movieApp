...
fetchDataFromServer(`https://api.themoviedb.org/3/genre/movie/list?api_key=${api_key}&language=uk-UA`, function ({ genres }) {
  ...
  fetchDataFromServer(`https://api.themoviedb.org/3/movie/popular?api_key=${api_key}&page=1&language=uk-UA`, heroBanner);
...
const heroBanner = function ({ results: movieList }) {
  ...
  banner.ariaLabel = "Популярні фільми";
...
<div class="slider-control">
  ...
  for (const [index, movie] of movieList.entries()) {
    ...
    <img src="${imageBaseURL}w154${poster_path}" alt="Прокрутити до ${title}" loading="lazy"
  ...
  for (const { title, path } of homePageSections) {
    fetchDataFromServer(`https://api.themoviedb.org/3${path}?api_key=${api_key}&page=1&language=uk-UA`, createMovieList, title);
  }
